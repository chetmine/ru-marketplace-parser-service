...
## 📊 Особенности парсинга маркетплейсов

В микросервисе реализована логика работы с различными маркетплейсами, учитывающая особенности их защиты:

| Маркетплейс     | Особенности и ограничения защиты                                      |
|-----------------|-----------------------------------------------------------------------|
| **Ozon**        | Важен только правильный **User-Agent**.                               |
| **WildBerries** | Иногда реагирует на VPN, но не всегда.                                |
| **Yandex Market**| **Очень строгая проверка!** Yandex-капча при использовании VPN. Неважен User-Agent. |
| **MegaMarket**  | **Очень строгая проверка.** Есть капча. Допускаются только российские IP-адреса, преимущественно резидентские прокси. |
| **Magnit Market**| Ограничений и строгой защиты не обнаружено.                            |
| **Alikson**     | Ограничений и строгой защиты не обнаружено.                            |

### Информация о прокси-провайдерах
Сервис спроектирован для работы с прокси-серверами. Текущие рекомендации/настройки для провайдеров:
- **WebShare**: 10 прокси, 1 ГБ трафика, безлимитный доступ, авторизация через пароль или IP.
- **ProxyScrape**: 100 прокси, безлимитный трафик, доступ на 7 дней, авторизация через IP.
