...
class MegaMarketParser extends MarketPlaceParser_1.MarketPlaceParser {
    // @ts-ignore
    constructor({ config }) {
        super();
        this.marketplaceUrl = "https://megamarket.ru";
    ...
    async parseProductInfo(page, productContainer) {
        const name = await this.safeFetchText(productContainer.locator(`header h1`));
        const priceString = await this.safeFetchText(productContainer.locator(`.sales-block-offer-price__price-final`));
        const oldPriceString = await this.safeFetchText(productContainer.locator(`.crossed-old-price-with-discount__crossed-old-price`), 50);
        const imgUrl = await productContainer.locator('img[class*="base-gallery-slide__inner-image-zoom-slide"]').first().getAttribute('src');
        const deliveryDate = await this.safeFetchText(productContainer.locator(`.sales-block-delivery-type__date`));
        let scoresInfo;
        const averageString = await this.safeFetchText(productContainer.locator(`.pui-rating-display__narrow-text`));
        const countString = await this.safeFetchText(productContainer.locator(`.pui-rating-display__text-bullet`));
    ...
    async parseFeature(featureElement) {
        const featureName = await this.safeFetchText(featureElement
            ...
            .first());
        const featureValue = await this.safeFetchText(featureElement
    ...
    async fetchProducts(page, product, isPublishResults) {
        ...
        const products = await Promise.all(productCards.map(card => this.parseProduct(card)));
        return products;
    }
    async parseProduct(card) {
        const name = await card.locator(`.catalog-item-regular-desktop__main-info`).locator('a').first().getAttribute('title');
        const priceString = await this.safeFetchText(card.locator('.catalog-item-regular-desktop__price'));
        const oldPriceString = await this.safeFetchText(card.locator('.crossed-old-price-discount span'), 50);
        const href = await card.locator('.catalog-item-regular-desktop__main-info').locator(`a`).first().getAttribute('href');
        const imgUrl = await card.locator('img[class*="pui-img"]').first().getAttribute('src');
        const deliveryDate = await this.safeFetchText(card.locator('button[class*="catalog-buy-button__button"]').first());
        const product = {
            name: name,
            price: Number.parseInt(priceString?.replace(/\s/g, "").replace("₽", "")),
            oldPrice: Number.parseInt(oldPriceString?.replace(/\s/g, "").replace("₽", "")),
...
exports.default = MegaMarketParser;
